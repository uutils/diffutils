...
use fluent::{self, FluentArgs, FluentBundle, FluentResource};
...
/// Helper function to find the uudiff locales directory from a utility's locales directory
fn find_uudiff_locales_dir(utility_locales_dir: &Path) -> Option<PathBuf> {
    ...
    let uudiff_locales = normalized_dir
        .parent()? // printenv
        .parent()? // uu
        .parent()? // src
        .join("uudiff")
    ...
    uudiff_locales.exists().then_some(uudiff_locales)
}
...
fn create_bundle(
    ...
    // Load common strings from uudiff locales directory
    try_add_resource_from(find_uudiff_locales_dir(locales_dir));
...
/// Create a bundle from embedded English locale files with common uudiff strings
fn create_english_bundle_from_embedded(
    ...
    // First, try to load common uudiff strings
    if let Some(uudiff_content) = get_embedded_locale("uudiff/en-US.ftl") {
        let uudiff_resource = parse_fluent_resource(uudiff_content)?;
        bundle.add_resource_overriding(uudiff_resource);
    }
...
///
/// ```
/// use uudiff::locale::get_message_with_args;
...
///
/// ```
/// use uudiff::locale::setup_localization;
///
...
fn get_locales_dir(p: &str) -> Result<PathBuf, LocalizationError> {
    #[cfg(debug_assertions)]
    {
        ...
        // from uudiff path, load the locales directory from the program directory
    ...
    ($id:expr, $($key:expr => $value:expr),+ $(,)?) => {
        {
            ...
            $(
                ...
                match value_str.parse::<i64>() {
                    Ok(num_val) => args.set($key, num_val),
                    Err(e) => {
                        if *e.kind() == std::num::IntErrorKind::PosOverflow {
                            // Keep as string if not a number
                            // float will round the number which is not the input anymore

...
fn project_root() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let uudiff_path = Path::new(&manifest_dir);

    // Navigate from src/uudiff to project root
    let project_root = uudiff_path
        .parent() // src/
...
fn detect_target_utility() -> Option<String> {
    ...
    println!("cargo:rerun-if-env-changed=UUDIFF_TARGET_UTIL");

    // First check if an explicit environment variable was set
    if let Ok(target_util) = env::var("UUDIFF_TARGET_UTIL") {
    ...
    if let Ok(target_dir) = env::var("CARGO_TARGET_DIR") {
        let config_path = Path::new(&target_dir).join("uudiff_target_util.txt");
    ...
    if let Ok(project_root) = project_root() {
        let config_path = project_root.join("target/uudiff_target_util.txt");
        if let Ok(content) = fs::read_to_string(&config_path) {
            let util_name = content.trim();
            if !util_name.is_empty() && util_name != "multicall" {
                return Some(util_name.to_string());
...
/// # Errors
///
/// Returns an error if the locales for `util_name` or `uudiff` cannot be found
...
fn embed_single_utility_locale(
    ...
    // Always embed uudiff locale file if it exists
    embed_component_locales(embedded_file, locales_to_embed, "uudiff", |locale| {
        project_root.join(format!("src/uudiff/locales/{locale}.ftl"))
    })?;
...
fn embed_all_utility_locales(
    ...
    if !src_uu_dir.exists() {
        // When src/uu doesn't exist (e.g., standalone uudiff from crates.io),
    ...
    // Also embed uudiff locale file if it exists
    embed_component_locales(embedded_file, locales_to_embed, "uudiff", |locale| {
        project_root.join(format!("src/uudiff/locales/{locale}.ftl"))
    })?;
...
fn embed_static_utility_locales(
    ...
    // First, try to embed uudiff locales - critical for common translations like "Usage:"
    embed_component_locales(embedded_file, locales_to_embed, "uudiff", |locale| {
